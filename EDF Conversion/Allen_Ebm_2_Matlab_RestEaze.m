...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ProtocolDIR = 'C:\Users\Administrator\Desktop\RestEaze EDF' ;%'Y:\RestEaze data\';     %
modDateAfter = '1-Nov-2015 9:03:00';  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if  strcmp(listPSGFolders(i,1).name, '.')~=1 & strcmp(listPSGFolders(i,1).name,'..')~=1 & strcmp(listPSGFolders(i,1).name,'.DS_Store')~=1
        ...
        if (datenum(listPSGFolders(i,1).date) > datenum(modDateAfter))
            ...
            if strcmp(RecordingFolderName(9:12),'mSIT')
                ...
                ShortName = RecordingFolderName(11:22);
                fid2 = fopen([curdir ShortName ' RestEaze-Events SleepStage.txt']);
            end
            ...
            if fid2 > 0 
                ...
                HypnogramStart = [RKstart(1:10) ' ' RKstart(12:19)];
            ...
            elseif fid2 == -99
                ...
                for ii = 5 : 8
                    ...
                    fid3 = fopen([curdir ShortName ' RestEaze-SIT' num2str(ii) '.txt']);
                    if fid3 > 1 
                        %Staging_Int = 0;
            ...
            [EDF] = EDF_read_jhmi_rev_101(edf_FulFile);
            ...
            if ~strcmp(RecordingFolderName(9:12),'mSIT') %Making sure it is an overnight trial
                fidAp=fopen([curdir ShortName ' RestEaze-Events Apnea.txt']);
                ...
                if fidAp ~=-1
                    ...
                    for ii=1:M
                        EventDataAp{ii,1} = [EventDataAp{ii,1}(1:10), ' ',EventDataAp{ii,1}(12:19)];
                    end       
                end
                EDF.CISRE_Apnea=EventDataAp;
                
                fidAr=fopen([curdir ShortName ' RestEaze-Events Arousal.txt']);
                ...
                if fidAr ~= -1
                    ...
                    %Extracts the date+military time (yyyy-mm-dd hh:mm:ss)
                    for ii=1:M
                        EventDataAr{ii,1} = [EventDataAr{ii,1}(1:10),' ',EventDataAr{ii,1}(12:19)];
                    end
                end
